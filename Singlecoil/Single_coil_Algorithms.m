function[] = Single_coil_Algorithms(fid, fid2,name,radial,kdata,b1,samp,Xtrue)
global r mk
mk=[];
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% ktslr %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[nx,ny,nt,nc]=size(y);
n=nx*ny;
[zbar_hat,flag,resNE,iter] = cgls_mean(@E_forw_for_mean,@E_back_for_mean, y,0,10e-36,10);
...
%%%%%%%%%%%%%%%%%%%%% Init Start %%%%%%%%%%%%%%%%%%
C_tilda=36;
alpha=C_tilda*norm(yinter(:))^2/(m*nt);
Y_trunc=yinter;
...
% tic;
for k=1:nt
    Ehat(:,:,k)=cgls_modi(@E_forw_for_Ak,@E_back_for_Ak,squeeze(yk(:,:,k,:)),0,1e-3,3);
end
% Time=toc;
% tic
...
fprintf(fid, '%s(%d) & %8.4f (%5.2f)& %8.4f (%5.2f) & %8.4f (%5.2f)& %8.4f (%5.2f)& %8.4f (%5.2f)& %8.4f (%5.2f) \n', name, radial,NMSE_ktslr, Time_ktslr,NMSE_Otazo, Time_Otazo,NMSE_lin, Time_lin,NMSE_GD,Time_GD,NMSE_MRI,Time_MRI,NMSE_MRI2,Time_MRI2);
fprintf(fid2, '%s(%d) & %8.4f (%5.2f)& %8.4f (%5.2f) & %8.4f (%5.2f)& %8.4f (%5.2f)& %8.4f (%5.2f)& %8.4f (%5.2f)  \n', name, radial,sim_ktslr, Time_ktslr,sim_Otazo, Time_Otazo,sim_lin,Time_lin,sim_GD,Time_GD,NMSE_MRI,Time_MRI,sim_MRI2,Time_MRI2);
end
