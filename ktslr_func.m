function [Zhat_ktslr,Time_ktslr ]=ktslr_func(kdata,b1c,samp)
tic;
...
o=0;
earray=zeros(1,opts.outer_iter*opts.inner_iter); 
cost=zeros(1,opts.outer_iter*opts.inner_iter);

fac=length((find(b~=0)));
U = double(U); b=double(b); 
fac=double(fac);
